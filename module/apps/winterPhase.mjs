...
export class PENWinter {
    ...
    for (let i of this.actor.items) {
      if (['skill', 'trait', 'passion'].includes(i.type)) {
        if (i.system.XP) {
          ...
          let newRes = {
            'type': game.i18n.localize("PEN." + i.type),
          ...
          await i.update({
            'system.winter': Number(i.system.winter) + Number(result.level),
            'system.XP': false
          })
          success.push(newRes);
        }
    ...
    for (let picked of selected) {
      ...
      if (picked.itemID === 'siz' && route != 'prestige') {
        ...
        await this.actor.update({
          'system.stats.siz.growth': this.actor.system.stats.siz.growth + 1,
  ...
  static async winterImproveTrait(route, event) {
    ...
    const traits = await (this.actor.items.filter(itm => itm.type === 'trait')).map(itm => {
      return {
  ...
  static async economic(event) {
    ...
    switch (newSOL) {
      ...
      case "poor":
        ...
        horses = await this.actor.items.filter(aitm => aitm.type === 'horse').filter(itm => !itm.system.poor).map(itm => {
          return {
            _id: itm.id,
    ...
    await this.actor.update({
      'system.stats.siz.sol': impoverished,
        ...
        switch (senResult) {
          ...
          case 5:
            if (senRes[senResult].base > 5) {
              ...
              results.push({
                label: senRes[senResult].label,
                number: senResult
              })
            } else {
              senRes[4].value++
              results.push({
                label: senRes[4].label,
                number: senResult
              })
            }
            break
